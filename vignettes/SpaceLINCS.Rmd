...
major calculation steps. First, for the so-called 'connectivities', we
calculate the rank correlation between the fold change values of the
experimental matrix and the fold change of genes under various
perturbagens (obtained from the LINCS L1000 database). We choose
computing the correlation (over other methods such as GSEA) as it is
fast because we need to calculate over tens of thousands of
profiles. The method also indirectly overcomes the limitation that the
L1000 signatures only measure 1000 genes by using the correlation at
the first level rather than using gene set based methods that are
slower but also ideally need the full set of genes. Then, we calculate
the connectivity enrichment using the GSEA method, where the
previously computed correlation coefficients are used as ranking
metric to perform a GSEA across all the datasets for a given
perturbagen. The two steps are illustrated in the code snippet below:
...
```
![The top 16 drugs correlated with the tested GE profile](images/plotDrugConnectivity.png "Drugs connectivity plots"){width="100%"}

### Plot the mechanism of action using plotMOA()

The function plotMOA() plots the mechanism-of-action enrichment
plots. When specifying the type as "drugClass", the barplot displays
the most positively and negatively enriched compound MoA/drug classes
(top figure). On the vertical axis, the GSEA normalized enrichment
score of the MoA class is plotted.

When specifying the type as "targetGene", the barplot displays the
most positively and negatively enriched target genes (bottom
figure). On the vertical axis, the GSEA normalized enrichment score of
the gene target is plotted.
...
![Mechanism-of-action of the top enriched drugs](images/plotMOA-drugClass.png "Drug class"){width="100%"}
![Target gene enrichment of the top enriched drugs](images/plotMOA-targetGene.png "Target gene"){width="100%"}
...
![Activation Matrix for Connectivity scores](images/plotActivationMap.png){width="100%"}
